        ...
        switch (buffer_in[0]) {
            ...
            case MSG_ID_OD_READ:
            {
                if (buffer_in_recv == sizeof(ipc_msg_od_t)) {
                    ipc_msg_od_t ipc_msg_od;
                    memcpy(&ipc_msg_od, buffer_in, sizeof(ipc_msg_od_t));
                    log_debug("od read index 0x%X subindex 0x%X", ipc_msg_od.index, ipc_msg_od.subindex);
                    OD_entry_t *entry = OD_find(od, ipc_msg_od.index);
                    OD_IO_t io = { 0 };
                    ODR_t r = OD_getSub(entry, ipc_msg_od.subindex, &io, false);
                    if (r == ODR_OK) {
                        memcpy(buffer_out, buffer_in, buffer_in_recv);
                        OD_get_value(entry, ipc_msg_od.subindex, &buffer_out[buffer_in_recv], 4, false);
            ...
            case MSG_ID_OD_WRITE:
            {
                if (buffer_in_recv > (int)sizeof(ipc_msg_od_t)) {
                    ipc_msg_od_t ipc_msg_od;
                    memcpy(&ipc_msg_od, buffer_in, sizeof(ipc_msg_od_t));
                    log_debug("od write index 0x%X subindex 0x%X", ipc_msg_od.index, ipc_msg_od.subindex);
                    OD_entry_t *entry = OD_find(od, ipc_msg_od.index);
                    ODR_t r = OD_set_value(entry, ipc_msg_od.subindex, &buffer_in[sizeof(ipc_msg_od_t)], buffer_in_recv - sizeof(ipc_msg_od_t), false);
                    if (r == ODR_OK) {
                        memcpy(buffer_out, buffer_in, buffer_in_recv);
                        buffer_out_send = buffer_in_recv;
                    } else {
            ...
            case MSG_ID_SDO_READ:
            {
                if (buffer_in_recv == sizeof(ipc_msg_sdo_t)) {
                    ipc_msg_sdo_t ipc_msg_sdo;
                    memcpy(&ipc_msg_sdo, buffer_in, sizeof(ipc_msg_sdo_t));
                    log_debug("sdo read index 0x%X subindex 0x%X", ipc_msg_sdo.index, ipc_msg_sdo.subindex);
                    void *data = NULL;
                    size_t data_len = 0;
                    CO_SDO_abortCode_t ac = -1;
                    if (CO && CO->SDOclient) {
                        ac = sdo_read_dynamic(CO->SDOclient, ipc_msg_sdo.node_id, ipc_msg_sdo.index, ipc_msg_sdo.subindex, &data, &data_len);
                    }
                    if (ac == 0) {
                        if (data == NULL) {
                            memcpy(buffer_out, buffer_in, buffer_in_recv);
            ...
            case MSG_ID_SDO_WRITE:
            {
                if (buffer_in_recv > (int)sizeof(ipc_msg_sdo_t)) {
                    ipc_msg_sdo_t ipc_msg_sdo;
                    memcpy(&ipc_msg_sdo, buffer_in, sizeof(ipc_msg_sdo_t));
                    log_debug("sdo write index 0x%X subindex 0x%X", ipc_msg_sdo.index, ipc_msg_sdo.subindex);
                    CO_SDO_abortCode_t ac = -1;
                    if (CO && CO->SDOclient) {
                        ac = sdo_write(CO->SDOclient, ipc_msg_sdo.node_id, ipc_msg_sdo.index, ipc_msg_sdo.subindex, &buffer_in[sizeof(ipc_msg_od_t)], buffer_in_recv - 6);
                    }
                    if (ac == 0) {
                        memcpy(buffer_out, buffer_in, buffer_in_recv);
                        buffer_out_send = buffer_in_recv;
