...
bool *str2buf_bool(const char *str) {
    ...
    if (value) {
        if (!strncmp(str, "true", strlen(str)) || !strncmp(str, "1", strlen(str))) {
            *value = true;
        } else if (!strncmp(str, "false", strlen(str)) || !strncmp(str, "0", strlen(str))) {
            *value = false;
        } else {
            free(value);
            value = NULL;
...
uint8_t *str2buf_bytes(const char *str, size_t *out_len) {
    if (!str || !out_len || !strlen(str) || (strlen(str) % 2)) {
        return NULL;
    }
